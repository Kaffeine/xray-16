...
void CScriptGameObject::ResetActionQueue()
{
	...
	if (!l_tpScriptMonster)
		ai().script_engine().script_log		(LuaMessageType::Error,"CSciptEntity : cannot access class member ResetActionQueue!");
	else
...
void CScriptGameObject::AddAction	(const CScriptEntityAction *tpEntityAction, bool bHighPriority)
{
	CScriptEntity		*l_tpScriptMonster = smart_cast<CScriptEntity*>(&object());
	if (!l_tpScriptMonster)
		ai().script_engine().script_log		(LuaMessageType::Error,"CSciptEntity : cannot access class member AddAction!");
	else
...
const CScriptEntityAction *CScriptGameObject::GetActionByIndex(u32 action_index)
{
	CScriptEntity	*l_tpScriptMonster = smart_cast<CScriptEntity*>(&object());
	if (!l_tpScriptMonster) {
		ai().script_engine().script_log			(LuaMessageType::Error,"CScriptEntity : cannot access class member GetActionByIndex!");
		return			(0);
	}
	else
...
CHelicopter* CScriptGameObject::get_helicopter	()
{
	...
	if (!helicopter) {
		ai().script_engine().script_log		(LuaMessageType::Error,"CGameObject : cannot access class member get_helicopter!");
		NODEFAULT;
	}
	return helicopter;
}
...
CHangingLamp* CScriptGameObject::get_hanging_lamp()
{
	...
	if (!lamp) {
		ai().script_engine().script_log		(LuaMessageType::Error,"CGameObject : it is not a lamp!");
		NODEFAULT;
	}
	return lamp;
}
...
CHolderCustom* CScriptGameObject::get_custom_holder()
{
	...
	if(!holder){
		ai().script_engine().script_log(LuaMessageType::Error,"CGameObject : it is not a holder!");
	}
	return holder;
}
...
LPCSTR CScriptGameObject::WhoHitName()
{
	...
	{
		ai().script_engine().script_log			(LuaMessageType::Error,"CScriptGameObject : cannot access class member  WhoHitName()");
		return			NULL;
	}
}
...
bool CScriptGameObject::CheckObjectVisibility(const CScriptGameObject *tpLuaGameObject)
{
	...
	if (entity_alive && !entity_alive->g_Alive()) {
		ai().script_engine().script_log	(LuaMessageType::Error,"CScriptGameObject : cannot check visibility of dead object!");
		return			(false);
	}
	...
	if (!script_entity) {
		...
		if (!actor) {
			ai().script_engine().script_log	(LuaMessageType::Error,"CScriptGameObject : cannot access class member CheckObjectVisibility!");
			return		(false);
		}
...
void CScriptGameObject::set_previous_point	(int point_index)
{
	...
	if (!monster)
		ai().script_engine().script_log		(LuaMessageType::Error,"CGameObject : cannot access class member set_previous_point!");
	else
...
void CScriptGameObject::set_start_point	(int point_index)
{
	CCustomMonster		*monster = smart_cast<CCustomMonster*>(&object());
	if (!monster)
		ai().script_engine().script_log		(LuaMessageType::Error,"CGameObject : cannot access class member set_start_point!");
	else
...
u32 CScriptGameObject::get_current_patrol_point_index()
{
	CCustomMonster		*monster = smart_cast<CCustomMonster*>(&object());
	if (!monster) {
		ai().script_engine().script_log		(LuaMessageType::Error,"CGameObject : cannot call [get_current_patrol_point_index()]!");
		return			(u32(-1));
	}
...
void CScriptGameObject::SetQueueSize(u32 queue_size)
{
	...
	if (!weapon) {
		ai().script_engine().script_log	(LuaMessageType::Error,"CWeaponMagazined : cannot access class member SetQueueSize!");
		return;
	}
...
u32	CScriptGameObject::Cost			() const
{
	...
	if (!inventory_item) {
		ai().script_engine().script_log			(LuaMessageType::Error,"CSciptEntity : cannot access class member Cost!");
		return			(false);
	}
...
float CScriptGameObject::GetCondition	() const
{
	CInventoryItem		*inventory_item = smart_cast<CInventoryItem*>(&object());
	if (!inventory_item) {
		ai().script_engine().script_log			(LuaMessageType::Error,"CSciptEntity : cannot access class member GetCondition!");
		return			(false);
	}
...
void CScriptGameObject::SetCondition	(float val)
{
	CInventoryItem		*inventory_item = smart_cast<CInventoryItem*>(&object());
	if (!inventory_item) {
		ai().script_engine().script_log			(LuaMessageType::Error,"CSciptEntity : cannot access class member SetCondition!");
		return;
	}
...
{
	if(!item) {
		ai().script_engine().script_log			(LuaMessageType::Error,"CSciptEntity : cannot access class member eat!");
		return;
	}
...
bool CScriptGameObject::inside					(const Fvector &position, float epsilon) const
{
	...
	if (!space_restrictor) {
		ai().script_engine().script_log			(LuaMessageType::Error,"CSpaceRestrictor : cannot access class member inside!");
		return			(false);
	}
	Fsphere				sphere;
	sphere.P			= position;
...
void CScriptGameObject::set_patrol_extrapolate_callback(const luabind::functor<bool> &functor)
{
	CCustomMonster			*monster = smart_cast<CCustomMonster*>(&object());
	if (!monster) {
		ai().script_engine().script_log			(LuaMessageType::Error,"CCustomMonster : cannot access class member set_patrol_extrapolate_callback!");
		return;
	}
...
void CScriptGameObject::extrapolate_length		(float extrapolate_length)
{
	CCustomMonster			*monster = smart_cast<CCustomMonster*>(&this->object());
	if (!monster) {
		ai().script_engine().script_log			(LuaMessageType::Error,"CCustomMonster : cannot access class member extrapolate_length!");
		return;
	}
...
void CScriptGameObject::set_fov					(float new_fov)
{
	CCustomMonster			*monster = smart_cast<CCustomMonster*>(&this->object());
	if (!monster) {
		ai().script_engine().script_log			(LuaMessageType::Error,"CCustomMonster : cannot access class member set_fov!");
		return;
	}
...
void CScriptGameObject::set_range				(float new_range)
{
	CCustomMonster			*monster = smart_cast<CCustomMonster*>(&this->object());
	if (!monster) {
		ai().script_engine().script_log			(LuaMessageType::Error,"CCustomMonster : cannot access class member set_range!");
		return;
	}
...
u32	CScriptGameObject::vertex_in_direction(u32 level_vertex_id, Fvector direction, float max_distance) const
{
	CCustomMonster	*monster = smart_cast<CCustomMonster*>(&object());
	if (!monster) {
		ai().script_engine().script_log			(LuaMessageType::Error,"CCustomMonster : cannot access class member vertex_in_direction!");
		return		(u32(-1));
	}

	if (!monster->movement().restrictions().accessible(level_vertex_id)) {
		ai().script_engine().script_log			(LuaMessageType::Error,"CCustomMonster::vertex_in_direction - start vertex id is not accessible!");
		return		(u32(-1));
	}
...
bool CScriptGameObject::invulnerable		() const
{
	CCustomMonster	*monster = smart_cast<CCustomMonster*>(&object());
	if (!monster) {
		ai().script_engine().script_log			(LuaMessageType::Error,"CCustomMonster : cannot access class member invulnerable!");
		return		(false);
	}
...
LPCSTR CScriptGameObject::get_smart_cover_description	() const {
	...
	if (!smart_cover_object) {
		ai().script_engine().script_log			(LuaMessageType::Error,"smart_cover::object : cannot access class member get_smart_cover_description!");
		return (0);
	}
