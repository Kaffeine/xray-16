...
const xr_vector<MemorySpace::CVisibleObject>	&CScriptGameObject::memory_visible_objects	() const
{
	...
	if (!monster) {
		ai().script_engine().script_log		(LuaMessageType::Error,"CGameObject : cannot access class member memory_visible_objects!");
		NODEFAULT;
	}
...
const xr_vector<MemorySpace::CSoundObject>	&CScriptGameObject::memory_sound_objects	() const
{
	CCustomMonster	*monster = smart_cast<CCustomMonster*>(&object());
	if (!monster) {
		ai().script_engine().script_log		(LuaMessageType::Error,"CGameObject : cannot access class member memory_sound_objects!");
		NODEFAULT;
	}
...
const xr_vector<MemorySpace::CHitObject>		&CScriptGameObject::memory_hit_objects		() const
{
	CCustomMonster	*monster = smart_cast<CCustomMonster*>(&object());
	if (!monster) {
		ai().script_engine().script_log		(LuaMessageType::Error,"CGameObject : cannot access class member memory_hit_objects!");
		NODEFAULT;
	}
...
void CScriptGameObject::ChangeTeam(u8 team, u8 squad, u8 group)
{
	...
	if (!custom_monster)
		ai().script_engine().script_log					(LuaMessageType::Error,"CCustomMonster: cannot access class member ChangeTeam!");
	else
...
CScriptGameObject *CScriptGameObject::GetEnemy() const
{
	...
	if (l_tpCustomMonster && l_tpCustomMonster->g_Alive() ) {
		...
		ai().script_engine().script_log			(LuaMessageType::Error,"CScriptGameObject : cannot access class member GetEnemy!");
		return			(0);
	}
}
...
CScriptGameObject *CScriptGameObject::GetCorpse() const
{
	...
	if (l_tpCustomMonster)
		...
		ai().script_engine().script_log			(LuaMessageType::Error,"CScriptGameObject : cannot access class member GetCorpse!");
		return			(0);
	}
}
...
bool CScriptGameObject::CheckTypeVisibility(const char *section_name)
{
	...
	if (l_tpCustomMonster)
		...
		ai().script_engine().script_log			(LuaMessageType::Error,"CScriptGameObject : cannot access class member CheckTypeVisibility!");
		return			(false);
	}
}
...
CScriptGameObject *CScriptGameObject::GetCurrentWeapon() const
{
	...
	if (!l_tpStalker) {
		ai().script_engine().script_log		(LuaMessageType::Error,"CAI_Stalker : cannot access class member GetCurrentWeapon!");
		return		(0);
	}
...
void CScriptGameObject::deadbody_closed(bool status)
{
	...
	if (!inventoryOwner) {
		ai().script_engine().script_log		(LuaMessageType::Error,"CInventoryOwner : cannot access class member deadbody_closed!");
		return;
	}
...
bool CScriptGameObject::deadbody_closed_status()
{
	CInventoryOwner		*inventoryOwner = smart_cast<CInventoryOwner*>(&object());
	if (!inventoryOwner) {
		ai().script_engine().script_log		(LuaMessageType::Error,"CInventoryOwner : cannot access class member deadbody_closed_status!");
		return		(0);
	}
...
void CScriptGameObject::can_select_weapon(bool status)
{
	...
	if(!stalker) 
	{
		ai().script_engine().script_log(LuaMessageType::Error,"CAI_Stalker : cannot access class member can_select_weapon!");
		return;
	}
...
void CScriptGameObject::deadbody_can_take(bool status)
{
	CInventoryOwner		*inventoryOwner = smart_cast<CInventoryOwner*>(&object());
	if (!inventoryOwner) {
		ai().script_engine().script_log		(LuaMessageType::Error,"CInventoryOwner : cannot access class member deadbody_can_take!");
		return;
	}
...
bool CScriptGameObject::deadbody_can_take_status()
{
	CInventoryOwner		*inventoryOwner = smart_cast<CInventoryOwner*>(&object());
	if (!inventoryOwner) {
		ai().script_engine().script_log		(LuaMessageType::Error,"CInventoryOwner : cannot access class member deadbody_can_take_status!");
		return		(0);
	}
...
CScriptGameObject *CScriptGameObject::GetCurrentOutfit() const
{
	CInventoryOwner		*inventoryOwner = smart_cast<CInventoryOwner*>(&object());
	if (!inventoryOwner) {
		ai().script_engine().script_log		(LuaMessageType::Error,"CInventoryOwner : cannot access class member GetCurrentOutfit!");
		return		(0);
	}
...
float CScriptGameObject::GetCurrentOutfitProtection(int hit_type)
{
	CInventoryOwner		*inventoryOwner = smart_cast<CInventoryOwner*>(&object());
	if (!inventoryOwner) {
		ai().script_engine().script_log		(LuaMessageType::Error,"CInventoryOwner : cannot access class member GetCurrentOutfitProtection!");
		return		(0);
	}
	CGameObject		*current_equipment = inventoryOwner->GetOutfit();
...
LPCSTR CScriptGameObject::GetPatrolPathName()
{
	...
	if (!stalker) {
		...
		if (!script_monster) {
			ai().script_engine().script_log		(LuaMessageType::Error,"CGameObject : cannot access class member GetPatrolPathName!");
			return		("");
		}
		else
...
void CScriptGameObject::add_animation			(LPCSTR animation, bool hand_usage, bool use_movement_controller)
{
	CAI_Stalker			*stalker = smart_cast<CAI_Stalker*>(&object());
	if (!stalker) {
		ai().script_engine().script_log		(LuaMessageType::Error,"CGameObject : cannot access class member add_animation!");
		return;
	}
	
	if (stalker->movement().current_params().cover()) {
		ai().script_engine().script_log(LuaMessageType::Error,"Cannot add animation [%s]: object [%s] is in smart_cover!", animation, stalker->cName().c_str());
	}
...
void CScriptGameObject::clear_animations		()
{
	CAI_Stalker			*stalker = smart_cast<CAI_Stalker*>(&object());
	if (!stalker) {
		ai().script_engine().script_log		(LuaMessageType::Error,"CGameObject : cannot access class member clear_animations!");
		return;
	}
...
void CScriptGameObject::set_dest_level_vertex_id(u32 level_vertex_id)
{
	CAI_Stalker					*stalker = smart_cast<CAI_Stalker*>(&object());
	if (!stalker)
		ai().script_engine().script_log					(LuaMessageType::Error,"CAI_Stalker : cannot access class member set_dest_level_vertex_id!");
	else {

		if (!ai().level_graph().valid_vertex_id(level_vertex_id)) {
#ifdef DEBUG
			ai().script_engine().script_log				(LuaMessageType::Error,"CAI_Stalker : invalid vertex id being setup by action %s!",stalker->brain().CStalkerPlanner::current_action().m_action_name);
#endif
			return;
		}
...
void CScriptGameObject::set_dest_game_vertex_id( GameGraph::_GRAPH_ID game_vertex_id)
{
	CAI_Stalker					*stalker = smart_cast<CAI_Stalker*>(&object());
	if (!stalker)
		ai().script_engine().script_log					(LuaMessageType::Error,"CAI_Stalker : cannot access class member set_dest_game_vertex_id!");
	else {
...
void CScriptGameObject::set_movement_selection_type(ESelectionType selection_type){
	CAI_Stalker					*stalker = smart_cast<CAI_Stalker*>(&object());
	if (!stalker)
		ai().script_engine().script_log					(LuaMessageType::Error,"CAI_Stalker : cannot access class member set_movement_selection_type!");
...
void  CScriptGameObject::set_desired_direction	(const Fvector *desired_direction)
{
	...
	if (!stalker)
		...
		if (fsimilar(desired_direction->magnitude(), 0.f))
			ai().script_engine().script_log				(LuaMessageType::Error,"CAI_Stalker : [%s] set_desired_direction - you passed zero direction!", stalker->cName().c_str());
		else {
			if (!fsimilar(desired_direction->magnitude(), 1.f))
				ai().script_engine().script_log			(LuaMessageType::Error,"CAI_Stalker : [%s] set_desired_direction - you passed non-normalized direction!", stalker->cName().c_str());
		}
...
MonsterSpace::EMovementType CScriptGameObject::movement_type			() const
{
	CAI_Stalker		*stalker = smart_cast<CAI_Stalker*>(&object());
	if (!stalker) {
		ai().script_engine().script_log					(LuaMessageType::Error,"CAI_Stalker : cannot access class member movement_type!");
...
MonsterSpace::EMovementType CScriptGameObject::target_movement_type		() const
{
	CAI_Stalker		*stalker = smart_cast<CAI_Stalker*>(&object());
	if (!stalker) {
		ai().script_engine().script_log					(LuaMessageType::Error,"CAI_Stalker : cannot access class member target_movement_type!");
		return		(MonsterSpace::eMovementTypeStand);
	}
...
MonsterSpace::EMentalState CScriptGameObject::mental_state				() const
{
	CAI_Stalker		*stalker = smart_cast<CAI_Stalker*>(&object());
	if (!stalker) {
		ai().script_engine().script_log					(LuaMessageType::Error,"CAI_Stalker : cannot access class member mental_state!");
...
DetailPathManager::EDetailPathType CScriptGameObject::detail_path_type	() const
{
	CAI_Stalker		*stalker = smart_cast<CAI_Stalker*>(&object());
	if (!stalker) {
		ai().script_engine().script_log					(LuaMessageType::Error,"CAI_Stalker : cannot access class member detail_path_type!");
...
u32	CScriptGameObject::GetInventoryObjectCount() const
{
	...
	if (l_tpInventoryOwner)
		...
		ai().script_engine().script_log			(LuaMessageType::Error,"CScriptGameObject : cannot access class member obj_count!");
		return			(0);
	}
}
...
CScriptGameObject	*CScriptGameObject::GetActiveItem()
{
	...
	if (l_tpInventoryOwner)
		...
		ai().script_engine().script_log			(LuaMessageType::Error,"CScriptGameObject : cannot access class member activge_item!");
		return			(0);
	}
}
...
CScriptGameObject	*CScriptGameObject::GetObjectByName	(LPCSTR caObjectName) const
{
	...
	if (l_tpInventoryOwner) {
		...
		ai().script_engine().script_log			(LuaMessageType::Error,"CScriptGameObject : cannot access class member object!");
		return			(0);
	}
}
...
bool CScriptGameObject::weapon_strapped	() const
{
	CAI_Stalker		*stalker = smart_cast<CAI_Stalker*>(&object());
	if (!stalker) {
		ai().script_engine().script_log	(LuaMessageType::Error,"CScriptGameObject : cannot access class member weapon_strapped!");
		return		(false);
	}
...
bool CScriptGameObject::weapon_unstrapped	() const
{
	CAI_Stalker		*stalker = smart_cast<CAI_Stalker*>(&object());
	if (!stalker) {
		ai().script_engine().script_log	(LuaMessageType::Error,"CScriptGameObject : cannot access class member weapon_unstrapped!");
		return		(false);
	}
...
bool CScriptGameObject::path_completed	() const
{
	CCustomMonster	*monster = smart_cast<CCustomMonster*>(&object());
	if (!monster) {
		ai().script_engine().script_log	(LuaMessageType::Error,"CScriptGameObject : cannot access class member path_completed!");
		return		(false);
	}
...
void CScriptGameObject::patrol_path_make_inactual	()
{
	CCustomMonster	*monster = smart_cast<CCustomMonster*>(&object());
	if (!monster) {
		ai().script_engine().script_log	(LuaMessageType::Error,"CScriptGameObject : cannot access class member patrol_path_make_inactual!");
		return;
	}
...
Fvector	CScriptGameObject::head_orientation		() const
{
	CAI_Stalker		*stalker = smart_cast<CAI_Stalker*>(&object());
	if (!stalker) {
		ai().script_engine().script_log	(LuaMessageType::Error,"CScriptGameObject : cannot access class member head_orientation!");
...
void CScriptGameObject::jump(const Fvector &position, float factor)
{
	...
	if (!monster) {
		ai().script_engine().script_log	(LuaMessageType::Error,"CScriptGameObject : cannot process jump for not a monster!");
		return;
	}
...
void CScriptGameObject::make_object_visible_somewhen	(CScriptGameObject *object)
{
	...
	if (!stalker) {
		ai().script_engine().script_log			(LuaMessageType::Error,"CAI_Stalker : cannot access class member make_object_visible_somewhen!");
		return;
	}

	CEntityAlive	*entity_alive = smart_cast<CEntityAlive*>(&object->object());
	if (!entity_alive) {
		ai().script_engine().script_log			(LuaMessageType::Error,"CEntityAlive : cannot access class member make_object_visible_somewhen!");
		return;
	}
...
void CScriptGameObject::sell_condition			(CScriptIniFile *ini_file, LPCSTR section)
{
	...
	if (!inventory_owner) {
		ai().script_engine().script_log			(LuaMessageType::Error,"CInventoryOwner : cannot access class member sell_condition!");
		return;
	}
...
void CScriptGameObject::buy_condition			(CScriptIniFile *ini_file, LPCSTR section)
{
	CInventoryOwner								*inventory_owner = smart_cast<CInventoryOwner*>(&object());
	if (!inventory_owner) {
		ai().script_engine().script_log			(LuaMessageType::Error,"CInventoryOwner : cannot access class member buy_condition!");
		return;
	}
...
void CScriptGameObject::show_condition			(CScriptIniFile *ini_file, LPCSTR section)
{
	CInventoryOwner								*inventory_owner = smart_cast<CInventoryOwner*>(&object());
	if (!inventory_owner) {
		ai().script_engine().script_log			(LuaMessageType::Error,"CInventoryOwner : cannot access class member show_condition!");
		return;
	}
...
void CScriptGameObject::buy_item_condition_factor(float factor)
{
	CInventoryOwner								*inventory_owner = smart_cast<CInventoryOwner*>(&object());
	if (!inventory_owner) {
		ai().script_engine().script_log			(LuaMessageType::Error,"CInventoryOwner : cannot access class member buy_item_condition_factor!");
		return;
	}
...
LPCSTR CScriptGameObject::sound_prefix			() const
{
	CCustomMonster							*custom_monster = smart_cast<CCustomMonster*>(&object());
	if (!custom_monster) {
		ai().script_engine().script_log		(LuaMessageType::Error,"CCustomMonster : cannot access class member sound_prefix!");
		return								(0);
	}
...
{
	if ( !object ) {
		ai().script_engine().script_log		(LuaMessageType::Error,"CGameObject : cannot access class member is_weapon_going_to_be_strapped (object passed is null)!");
		return								false;
	}

	CAI_Stalker const *stalker				= smart_cast<CAI_Stalker const*>( &this->object() );
	if (!stalker) {
		ai().script_engine().script_log		(LuaMessageType::Error,"CGameObject : cannot access class member is_weapon_going_to_be_strapped!");
		return								false;
	}
