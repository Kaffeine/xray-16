...
struct CLoader {
        struct CHelperLoadStlContainer {
		template <typename T>
		...
		{
                    constexpr bool isTreeStructure = is_tree_structure<T1>::value;
                    if (isTreeStructure)
                        data.insert		(value);
                    else
	...
	IC	static void load_data(T &data, M &stream, const P &p)
	{
            constexpr bool isStl = object_type_traits::is_stl_container<T>::value;
            constexpr bool isPointer = !isStl && object_type_traits::is_pointer<T>::value;
            constexpr bool isSerializable = !isPointer && object_type_traits::is_base_and_derived<ISerializable,T>::value;

            if (isStl)
                CHelperLoadStlContainer::load_data(data,stream,p);
            else if (isPointer)
